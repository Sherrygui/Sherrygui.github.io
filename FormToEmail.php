...
You can get the Pro version of this script here: http://formtoemail.com/php_form_script.php
---------------------------------------------------------------------------------------------------
...
function build_message($request_input){if(!isset($message_output)){$message_output ="";}if(!is_array($request_input)){$message_output = strip_tags($request_input);}else{foreach($request_input as $key => $value){if(!empty($value)){if(!is_numeric($key)){$message_output .= str_replace("_"," ",ucfirst($key)).": ".build_message($value).PHP_EOL.PHP_EOL;}else{$message_output .= build_message($value).", ";}}}}return rtrim($message_output,", ");}
...
$subject = "Message from your website: " . $_REQUEST['subject'];
...
<html lang="en">
  ...
  <body>
    ...
    <main id="content">
      <div class="container">
        <div class="thank-you">
          <p class="success">Thank you for your message. I&rsquo;ll be in touch soon.</p>
        </div>
      </div>
    </main>
